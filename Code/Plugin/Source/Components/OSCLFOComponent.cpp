...
IVSTParameters(ID)
{
	...
	addAndMakeVisible(__lpSelect = new SelectKnob(*Global->paramHandler->Get<AudioParameterChoice>(ID, "FILTER_LP_LFO"), global));
	...
	addAndMakeVisible(__hpSelect = new SelectKnob(*Global->paramHandler->Get<AudioParameterChoice>(ID, "FILTER_HP_LFO"), global));
